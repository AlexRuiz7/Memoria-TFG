...
\begin{enumerate}
\item Se ha conectado el limitador LM7 a corriente y se han conectado a él un monitor (interfaz \glsname{VGA}) y un teclado (interfaz PS/2).
...
\begin{center}
    ...
    \captionof{figure}{Estructura de las claves en el fichero \textbf{\texttt{/etc/shadow}}}
    \label{fig:shadow}
\end{center}
...
Como componentes que no forman parte de la placa base, tenemos una tarjeta de sonido \glsname{USB} conectada a uno de los puertos y un circuito integrado para las entradas y salidas de audio, conectado a la placa base mediante su puerto serie.
...
Tal y como se comentó en la primera sección de este capítulo, el ordenador auxiliar que se ha instalado en el laboratorio, y en red como el \gls{LM7}, tiene \gls{WINDOWS} 10 como sistema operativo. Para facilitar el acceso remoto mediante \acrshort{SSH} al equipo limitador, se hace uso de clientes \acrshort{SSH}. En la tabla \ref{tab:gestoresSSH} se lista el cliente usado tanto en \gls{WINDOWS} 10 como en \glsname{GNU/Linux}.
